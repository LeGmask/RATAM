...
(* Nécessaire aux compilateurs intermédiaires (non complets) *)
module PasseTypeNop :
  Passe with type t1 = Ast.AstTds.programme and type t2 = Ast.AstType.programme =
struct
...
(* Nécessaire aux compilateurs intermédiaires (non complets) *)
module PasseCodeNop :
  Passe with type t1 = Ast.AstPlacement.programme and type t2 = string = struct
...
module VerifPlacement = struct
  ...
  let analyser (Ast.AstPlacement.Programme (_, fonctions, (prog, _), _)) =
    (* TODO add test globales + locales ? *)
